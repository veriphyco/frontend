...
const registrationSchema = Yup.object().shape({
  ...
  phoneNumber: Yup.string().phone("KE", true, 'Mobile Number not valid')
...
export function Registration() {
  ...
  const [serverResponseSuccess, setserverResponseSuccess] = useState()
  const [serverResponseFail, setserverResponsefail] = useState() 
      ...
      try {
        const sendData = async () => {
          try {
            ...
            const firstName = valuestToSend.firstName
            const lastName = valuestToSend.lastName
            const email = valuestToSend.email
            const phoneNumber = valuestToSend.phoneNumber 
            const country = valuestToSend.country
           

            // console.log(firstName)
            const data = await axios.post('/users/onboarding/register', {
              firstName,
              lastName,
              email,
              phoneNumber,
          ...
          //  console.log(data.data._id)
            setserverResponseSuccess('')
            setserverResponsefail('')
            console.log(data)
            if (data.data) {
              setserverResponseSuccess('Registration Successfull.. Please Proceed...')
              setLoading(false)
              let userID= data.data._id;
              console.log(userID)
              navigate('/stkpush', { state: { userID, phoneNumber, email } })
            //   let userID= data.data._id;
            //   console.log(data.data._id)
            // //  navigate('/stkpush', { state: { userID, phoneNumber, email } })
            //   setLoading(false)
  ...
  // const [frontIdimgupload, setfrontIdimg]= useState("fsdfsf");
  // const [backIdimgupload, setbackIdimgupload]= useState();
  // const [krapinimgupload, setkrapinimgupload]= useState();
  // const [mpesastatement, setmpesastatement]= useState();
  ...
  return (
    ...
    >
      ...
      <div className='mb-10 text-center'>
        ...
        <h1 className='text-dark mb-3'>Please the fill your  details below</h1>
        {/* end::Title */}

        {/* begin::Link */}
        <div className='text-gray-400 fw-bold fs-4'>
          Fill the form below for a quick and easy onboarding process
      ...
      <div className='fv-row mb-7'>
        <label className='form-label fw-bolder text-dark fs-6'>Phone Number format(254 7xx xxx xxx)</label>
        <input
           placeholder='e.g 254707268461'
          type='tel'
      ...
      {/* end::Form group Coutry */}
      {/* start::Form group id upload */}
     
      {/* start::Form group id upload */}
      {/* start::Form group id upload */}
      
      {/* start::Form group id upload */}
      {/* start::Form group KRA upload */}
    
      {/* start::Form group KRA upload */}
      {/* start::Form group MPESA statement upload */}
      ...
      {/* end::Form group */}
      {/* success/fail messge from the Server starts here*/}
      ...
      {/* success/fail messge from the Server ends here*/}
      {/* begin::Form group */}
